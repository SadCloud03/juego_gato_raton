...
def jugar():
    ...
    pygame.display.set_caption("Juego del Gato y el Ratón")
    reloj = pygame.time.Clock()
# Aun no logre hacer cargar las imagenes, estamos trabajando en eso
    # Cargar las imágenes de los GIFs
    # imagen_gato = pygame.image.load('static/gato.gif').convert_alpha()
    # imagen_gato = pygame.transform.scale(imagen_gato, (TAMANIO_CELDA, TAMANIO_CELDA))

    # imagen_raton = pygame.image.load('static/raton.gif').convert_alpha()
...
# Dibujar el tablero
        for x in range(TABLERO_TAMANIO):
            for y in range(TABLERO_TAMANIO):
                ...
                if tablero[x, y] == 1:
                    pygame.draw.circle(pantalla, COLOR_GATO, rect.center, TAMANIO_CELDA // 3)
                elif tablero[x, y] == 2:
                     pygame.draw.circle(pantalla, COLOR_RATON, rect.center, TAMANIO_CELDA // 3)
